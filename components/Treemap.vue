...
children: [
    ...
    { name: 'Ripple', value: 100 },
    { name: 'Litecoin', value: 1000 },
    { name: 'Bitcoin Cash', value: 2000 },
    { name: 'Dash', value: 150 },
    // More data...
],
});
...
const totalValue = computed(() => {
  return cryptos.value.reduce((acc, crypto) => acc + crypto.price, 0);
});
...
function getWidth(value) {
  const totalSurface = width * height; // Total surface area of the container
  const cryptoSurface = (value / totalValue.value) * totalSurface;
  return `${Math.sqrt(cryptoSurface) * 4}px`; // Adjust the multiplier for desired effect
}
function getHeight(value) {
  const totalSurface = width * height; // Total surface area of the container
  const cryptoSurface = (value / totalValue.value) * totalSurface;
  return `${Math.sqrt(cryptoSurface) * 3}px`; // Adjust the multiplier for desired effect
}
...
.treemap-item {
    ...
    padding: 10px;
    border-radius: 8px;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1), inset 0 2px 4px rgba(255, 255, 255, 0.2); /* Outer and inner shadows */
    color: white;
    font-weight: bold;
...
.treemap-item:hover {
    ...
    box-shadow: 0 6px 12px rgba(0, 0, 0, 0.2), inset 0 2px 4px rgba(255, 255, 255, 0.3); /* Enhanced shadows on hover */
}
.crypto-name {
    font-size: 16px;
}
